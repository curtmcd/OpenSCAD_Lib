...
function measureTextOffsets(text="", font="Liberation Sans", size=10., spacing=1., fonts=FONTS) =
    let(f=findFont(FONTS, font),
	factor=spacing * size * fontScale(f))
    [for (offset = getOffsets(text, f)) offset * factor];
